...
const historicFetches = (sym,id,dispatch) => {
  const obj = {};
  const check = () => {
    const completed = ((Boolean(obj.price) && Boolean(obj.daily) && Boolean(obj.weekly) && Boolean(obj.monthly) && Boolean(obj.yearly) &&  Boolean(obj.trimonthly) && Boolean(obj.fiveyearly)));
    if (completed) {
